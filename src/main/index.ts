...
app.whenReady().then(async () => {
  ...
  scheduler = await ReminderScheduler.restore(store);
  scheduler.start();
  ...
  tray = new TrayController({
    ...
    pause: () => void scheduler.dispatch({ type: "PAUSE", now: Date.now(), durationMs: 30 * 60_000 }),
    quit: () => app.quit()
  });
  tray.create();
  ...
  await createWindow();
  tray.refresh(scheduler.getState());
  await alerts.stateChanged(scheduler.getState());
